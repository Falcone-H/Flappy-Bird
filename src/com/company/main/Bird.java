...
public class Bird {
    ...
    public static BufferedImage[][] birdImages; //小鸟的图片数组对象
    private final int x;
    private int y;  //小鸟的坐标
    ...
    public Bird() {
        ...
        for (int j = 0; j < STATE_COUNT; j++) {
            for (int i = 0; i < IMG_COUNT; i++) {
                birdImages[j][i] = GameUtil.loadBufferedImage(Constant.BIRDS_ING_PATH[j][i]);
            }
        }
    ...
    public void draw(Graphics g) {
        ...
        g.drawImage(image, x - halfImgWidth, y - halfImgHeight, null);  //x坐标于窗口的1/4处，y坐标位于窗口中心
    ...
    private boolean keyFlag = true; //按键状态，true为已释放，使按住按键时不会重复调用方法
    ...
    public void birdDead() {
        ...
        countScore.isSaveScore();   //判断是否保存记录
    }
    ...
    //绘制实时分数
    private void drawScorce(Graphics g){
